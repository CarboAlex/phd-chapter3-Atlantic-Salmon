...
deltaT_stj_1sw <- read_fst(file.path(base_path, "03_Clean_Data/Sea/DeltaT_SST_River_stj_1SW.fst")) %>%
  dplyr::select(river, cohort = year, delta_T_sst_minus_river) %>%
  filter(!is.na(delta_T_sst_minus_river))

deltaT_stj_2sw <- read_fst(file.path(base_path, "03_Clean_Data/Sea/DeltaT_SST_River_stj_2SW.fst")) %>%
  dplyr::select(river, cohort = year, delta_T_sst_minus_river) %>%
  filter(!is.na(delta_T_sst_minus_river))

deltaT_tri_1sw <- read_fst(file.path(base_path, "03_Clean_Data/Sea/DeltaT_SST_River_tri_1SW.fst")) %>%
  dplyr::select(river, cohort = year, delta_T_sst_minus_river) %>%
  filter(!is.na(delta_T_sst_minus_river))

deltaT_tri_2sw <- read_fst(file.path(base_path, "03_Clean_Data/Sea/DeltaT_SST_River_tri_2SW.fst")) %>%
  dplyr::select(river, cohort = year, delta_T_sst_minus_river) %>%
  filter(!is.na(delta_T_sst_minus_river))


# ─────────────────────────────────────────────
# 2. Capelin_LabradorSea
#    Garder : toutes les colonnes sauf sw_type
#    Retirer les NA dans Capelin_Imputed_tp1 / tp2
# ─────────────────────────────────────────────

capelin_stj_1sw <- read_fst(file.path(base_path, "03_Clean_Data/Sea/Capelin_LabradorSea_stj_1SW.fst")) %>%
  dplyr::select(-sw_type) %>%
  filter(!is.na(Capelin_Imputed_tp1))

capelin_stj_2sw <- read_fst(file.path(base_path, "03_Clean_Data/Sea/Capelin_LabradorSea_stj_2SW.fst")) %>%
  dplyr::select(-sw_type) %>%
  filter(!is.na(Capelin_Imputed_tp2))

capelin_tri_1sw <- read_fst(file.path(base_path, "03_Clean_Data/Sea/Capelin_LabradorSea_tri_1SW.fst")) %>%
  dplyr::select(-sw_type) %>%
  filter(!is.na(Capelin_Imputed_tp1))

capelin_tri_2sw <- read_fst(file.path(base_path, "03_Clean_Data/Sea/Capelin_LabradorSea_tri_2SW.fst")) %>%
  dplyr::select(-sw_type) %>%
  filter(!is.na(Capelin_Imputed_tp2))


# ─────────────────────────────────────────────
# 3. Plankton_LabradorSea
#    Garder : toutes les colonnes sauf sw_type
#    Retirer les NA dans Plankton_t ou Plankton_tp1
#    (1SW n'a pas Plankton_tp1 -> filtrer seulement sur Plankton_t)
# ─────────────────────────────────────────────

plankton_stj_1sw <- read_fst(file.path(base_path, "03_Clean_Data/Sea/Plankton_LabradorSea_stj_1SW.fst")) %>%
  dplyr::select(-sw_type) %>%
  filter(!is.na(Plankton_t))

plankton_stj_2sw <- read_fst(file.path(base_path, "03_Clean_Data/Sea/Plankton_LabradorSea_stj_2SW.fst")) %>%
  dplyr::select(-sw_type) %>%
  filter(!is.na(Plankton_t) & !is.na(Plankton_tp1))

plankton_tri_1sw <- read_fst(file.path(base_path, "03_Clean_Data/Sea/Plankton_LabradorSea_tri_1SW.fst")) %>%
  dplyr::select(-sw_type) %>%
  filter(!is.na(Plankton_t))

plankton_tri_2sw <- read_fst(file.path(base_path, "03_Clean_Data/Sea/Plankton_LabradorSea_tri_2SW.fst")) %>%
  dplyr::select(-sw_type) %>%
  filter(!is.na(Plankton_t) & !is.na(Plankton_tp1))
...
# 4. Food_Index
#    Garder : toutes les colonnes sauf sw_type
#    Retirer les NA dans food_index
# ─────────────────────────────────────────────

food_stj_1sw <- read_fst(file.path(base_path, "03_Clean_Data/Sea/Food_Index_stj_1SW.fst")) %>%
  dplyr::select(-sw_type) %>%
  filter(!is.na(food_index))

food_stj_2sw <- read_fst(file.path(base_path, "03_Clean_Data/Sea/Food_Index_stj_2SW.fst")) %>%
  dplyr::select(-sw_type) %>%
  filter(!is.na(food_index), !is.na(plankton_index))

food_tri_1sw <- read_fst(file.path(base_path, "03_Clean_Data/Sea/Food_Index_tri_1SW.fst")) %>%
  dplyr::select(-sw_type) %>%
  filter(!is.na(food_index))

food_tri_2sw <- read_fst(file.path(base_path, "03_Clean_Data/Sea/Food_Index_tri_2SW.fst")) %>%
  dplyr::select(-sw_type) %>%
  filter(!is.na(food_index), !is.na(plankton_index))
...
th_stj_1sw <- read_fst(file.path(base_path, "03_Clean_Data/Sea/TH_annual_stj_1sw.fst")) %>%
  mutate(river = "stj") %>%
  filter(if_all(-cohort, ~ !is.na(.)))

th_stj_2sw <- read_fst(file.path(base_path, "03_Clean_Data/Sea/TH_annual_stj_2sw.fst")) %>%
  mutate(river = "stj") %>%
  filter(if_all(-cohort, ~ !is.na(.)))

th_tri_1sw <- read_fst(file.path(base_path, "03_Clean_Data/Sea/TH_annual_tri_1sw.fst")) %>%
  mutate(river = "tri") %>%
  filter(if_all(-cohort, ~ !is.na(.)))

th_tri_2sw <- read_fst(file.path(base_path, "03_Clean_Data/Sea/TH_annual_tri_2sw.fst")) %>%
  mutate(river = "tri") %>%
  filter(if_all(-cohort, ~ !is.na(.)))
