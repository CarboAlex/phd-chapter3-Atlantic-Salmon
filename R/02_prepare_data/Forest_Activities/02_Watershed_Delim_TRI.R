...
# ══════════════════════════════════════════════════════════════════════════════
#
# Downloads a DEM and delineates the watershed of the Trinite River using
...
#
# Outputs:
#   - R/01_derived_data/ForestActivities/dem_utm_TRI.tif
#       DEM reprojected to EPSG:32198
#   - R/01_derived_data/ForestActivities/dem_filled_TRI.tif
#       Depression-filled DEM
#   - R/01_derived_data/ForestActivities/d8_pointer_TRI.tif
#       D8 flow direction raster
#   - R/01_derived_data/ForestActivities/flow_accum_TRI.tif
#       D8 flow accumulation raster (cells)
#   - R/01_derived_data/ForestActivities/outlet_TRI.shp
#       Lowest point along the river line above 5 m elevation (pour point)
#   - R/01_derived_data/ForestActivities/snapped_outlet_TRI.shp
#       Pour point snapped to the highest-accumulation cell within 200 m
#   - R/01_derived_data/ForestActivities/raster_watershed_TRI.tif
...
out_river <- "TRI"                                  # River identifier tag

# Bounding box covering the Trinite watershed (WGS84)
bbx <- c(
  xmin = -67.878338,
  ymin =  49.359639,
  xmax = -67.160968,
...
elev_min  <- 5    # Minimum elevation (m) to exclude tidal/sea-level points
...
river_osm <- opq(bbox = bbx, timeout = 120) |>
...
river_pts_geom <- st_line_sample(river_line, density = 1 / 50) |>
  st_cast("POINT")
river_pts <- st_sf(geometry = river_pts_geom)

# Extract DEM elevation at each point
river_pts_vect <- vect(river_pts)
elev           <- terra::extract(dem, river_pts_vect)[, 2]

# Filter out points at or below sea level before selecting the outlet,
# then select the lowest remaining point
river_pts <- river_pts %>%
  dplyr::mutate(elevation = elev) %>%
  dplyr::filter(elevation > elev_min)
outlet <- river_pts[which.min(river_pts$elevation), ]
